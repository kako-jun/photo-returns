...
[sss（Smart Slide Show）](https://github.com/kako-jun/sss) は、整理済みの写真フォルダをスライドショーで消化するツールです。10万枚以上のコレクションでも完全平等ランダムアルゴリズムにより、どの写真にも均等に出番が来ます。

**推奨フロー:**
1. **photo-returns** でカメラから取り込んだ写真を年月日フォルダへ整理
2. **sss** で整理済みフォルダをスライドショーとして鑑賞

photo-returns で整理された `YYYY/YYYYMM/YYYYMMDD` 構造は sss との相性が良く、撮影日時ごとに思い出を振り返ることができます。
